...
def filterDict(paramDict,isKeys,hide):
    ...
    for pos in range(len(hideThese)):
...
def prepSingle(params,qsubList,splitList,passedX,passedY,passedC,lineIndex, subpopLoaded, useSubpop, multiCore):
    ...
    tracker = [0] * width
    for pos1 in range(limit):
	word = qsubList[pos1].replace(directoryIn,'')
	allFound = True
	for pos3 in range(cLen):
    ...
    for pos1 in range(limit):
        ...
        if keepLine:
            ...
            temp = checkLines(dirMatrix[xPos][yPos], subpopLoaded, useSubpop, multiCore, crossTalk)
...
def main():
    ...
    while column < len(script[0]):
        ...
        if useSubpop and len(params[15]) > 0:
            subpopFolder = '/'.join(subpopDir.split('/')[0:-1])
            crossTalk = params[15].lower
            for pos in range(len(crossTalk)):
                crossTalk[pos] = subpopFolder + crossTalk[pos]
            print crossTalk
            quit()
        else:
            crossTalk = 'null'
	...
	hideThese = params[16]
        ...
        if runAll:
            ...
            for item in qsubList:
                ...
                data = checkLines(item+'/'+target, subpopLoaded, useSubpop, multiCore, crossTalk)
